...
const LiffHomePage = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      {/* Compact Header - 固定在顶部，始终存在，用 opacity 控制 */}
      <div 
        className={`fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md shadow-md transition-all duration-300 ease-out ${isCompact ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-full pointer-events-none'}`}
      >
        <div className="px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-extrabold text-gray-900 text-lg">{greeting}，{displayName}</h1>
          </div>
          <button
      ...
      <div className="bg-white shadow-sm rounded-b-[40px] overflow-hidden">
        ...
        ></div>
        <div 
          className="absolute top-[-10%] right-[-10%] w-[60%] h-[80%] rounded-[100%] blur-3xl opacity-15 pointer-events-none" 
          style={{ backgroundColor: themeColor }}
        ></div>
        <div 
          className="absolute top-[20%] left-[-10%] w-[40%] h-[40%] rounded-full blur-2xl opacity-5 pointer-events-none" 
          style={{ backgroundColor: themeColor }}
        ></div>
        
        <div className="relative z-10 p-6 pt-10 pb-12">
          <div className="flex items-center justify-between w-full mb-8">
            <div>
              <h1 className="font-extrabold text-gray-900 tracking-tight text-3xl mb-2">
